...
\tightlist
\item
  \(\texttt{serialize}(P)\): Encodes a point in compressed form. The
...
\item
  \(\texttt{serialize}(s)\): Encodes a scalar into 32 octets in
  little-endian representation.
\item
  \(\texttt{le32}(n)\): Encode integer \(n\) as a 4-byte little-endian
  octet string.
\item
  \(\texttt{from\_le\_bytes\_mod\_order}(buf)\): Interpret octet string
  \(buf\) as a little-endian integer and reduce modulo the prime field
  order \(r\).
\end{itemize}
...
\item
  ...
  \(\lceil(\lceil\log_2(r)\rceil + 128) / 8\rceil\) = 48 bytes for
  Bandersnatch.
\end{itemize}
...
\texttt{"Bandersnatch\_XMD:SHA-512\_ELL2\_RO\_"} and
\(\texttt{suite\_string}\) = \texttt{"Bandersnatch\_SHA-512\_ELL2"}
(section 1.4).
...
\tightlist
\item
  \(T \gets \texttt{new}(\text{SUITE\_ID})\)
\item
  \(T.\texttt{absorb}(\texttt{PointToHash})\)
\item
  \(T.\texttt{absorb}(\texttt{serialize}(O))\)
\item
...
\emph{Initialization}: \(\texttt{new}(label)\) creates a fresh SHA-512
state and feeds \(\texttt{le32}(\texttt{len}(label)) \;\Vert\; label\)
into it.
...
\item
  ...
  \item
    ...
    squeeze \(n\) scalars
    \(z_i \gets \texttt{from\_le\_bytes\_mod\_order}(T'.\texttt{squeeze}(\texttt{challenge\_len}))\),
    \(I_m \gets \sum_{i} z_i \cdot I_i\),
    \(O_m \gets \sum_{i} z_i \cdot O_i\)
  \end{itemize}
\item
  \(T.\texttt{absorb}(\texttt{le32}(\texttt{len}(ad)) \;\Vert\; ad)\)
\item
  Return \((T, (I_m, O_m))\)
\end{enumerate}
...
\item
  \(T' \gets T.\texttt{fork}()\)
\item
  \(T'.\texttt{absorb}(\texttt{NonceExpand} \;\Vert\; \texttt{serialize}(sk))\)
\item
  \(h \gets T'.\texttt{squeeze}(64)\)
\item
  \(T.\texttt{absorb}(\texttt{Nonce} \;\Vert\; h[32..64])\)
\item
  \(k \gets \texttt{from\_le\_bytes\_mod\_order}(T.\texttt{squeeze}(\text{expanded\_scalar\_len}))\)
\end{enumerate}
...
\item
  ...
  \(T.\texttt{absorb}(\texttt{serialize}(P_i))\)
\item
  \(c \gets \texttt{from\_le\_bytes\_mod\_order}(T.\texttt{squeeze}(\texttt{challenge\_len}))\)
\end{enumerate}
...
\item
  ...
  \(\quad T_k.\texttt{absorb}(\texttt{serialize}(b))\),
  \(\quad k \gets \texttt{nonce}(x, T_k)\)
\item
  \(T_{kb} \gets T.\texttt{fork}()\),
  \(\quad T_{kb}.\texttt{absorb}(\texttt{serialize}(x))\),
...
(section 4.2, step 2) and forwarded to the ring prover.
...
that produces uniformly distributed scalars in the field \(\bold{F}\).
...
\item
  \(cnt \gets 0\)
\item
  \(T \gets \texttt{new}(\text{"ark-vrf-keygen"})\)
\item
  \(T.\texttt{absorb}(seed)\)
\item
  If \(cnt > 0\): \(T.\texttt{absorb}([cnt])\)
\item
  \(secret \gets \texttt{nonce}(0, T)\)
\item
...
The \(\texttt{nonce}\) procedure (section 1.12) is called with a zero
scalar and provides domain separation, uniform scalar derivation, and
negligible modular reduction bias.

\subsection{A.2. Deterministic Blinding Factor
Generation}\label{a.2.-deterministic-blinding-factor-generation}

For Pedersen VRF, the blinding factor may be generated deterministically
from the secret key and the VRF transcript state. This procedure uses
the nonce function (section 1.12) with a distinct domain separator.
