...
\item
  If \(n = 0\): return \((T, (\mathcal{O}, \mathcal{O}))\)
\item
  If \(n = 1\): return \((T, (I_0, O_0))\)
\item
  \(T' \gets T.\texttt{fork}(),\ T'.\texttt{absorb}(\texttt{Delinearize})\)
\item
  For \(i = 0, \ldots, n-1\):
...
\tightlist
\item
  \((T, (I_m, O_m)) \gets \texttt{delinearize}(\overline{io})\)
\item
  \(T.\texttt{absorb}(scheme \;\Vert\; \texttt{enc\_32}(\texttt{len}(ad)) \;\Vert\; ad)\)
\item
  Return \((T, (I_m, O_m))\)
\end{enumerate}
...
\tightlist
\item
  \(T.\texttt{absorb}(\texttt{Challenge} \;\Vert\; \texttt{enc\_point}(P_0) \;\Vert\; \cdots \;\Vert\; \texttt{enc\_point}(P_{m-1}))\)
\item
...
\href{https://datatracker.ietf.org/doc/rfc9380}{RFC-9380} {[}5{]}, with
...
When \(n = 0\), the \(\texttt{delinearize}\) procedure (section 1.6.4)
sets the merged pair to the identity:
