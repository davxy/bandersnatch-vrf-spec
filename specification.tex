...
\subsection{1.3. Constants}\label{constants}
...
\subsection{1.4. Codec}\label{codec}
...
\subsection{1.5. Procedures}\label{procedures}

\subsubsection{1.5.1. Transcript}\label{transcript}
...
\subsubsection{1.5.2. VRF Input}\label{vrf-input}
...
\subsubsection{1.5.3. VRF Output}\label{vrf-output}
...
\subsubsection{1.5.4. VRF-AD}\label{vrf-ad}
...
\subsubsection{1.5.5. VRF Transcript}\label{vrf-transcript}

Shared transcript construction used by all VRF-AD schemes. Absorbs
input/output pairs, derives delinearization scalars, merges pairs into a
single pair, and absorbs additional data.
...
\subsubsection{1.5.6. Nonce}\label{nonce}
...
\item
  \(T' \gets T.\texttt{fork}()\)
\item
  \(T'.\texttt{absorb}(\texttt{NonceExpand} \;\Vert\; \texttt{enc\_scalar}(sk))\)
\item
...
\subsubsection{1.5.7. Challenge}\label{challenge}
...
\item
  \(k_b \gets k \cdot G\), \(\quad k_h \gets k \cdot I_m\)
\item
  \(c \gets \texttt{challenge}([Y, k_b, k_h], T)\)
\item
  \(s \gets k + c \cdot x\)
\item
  \(\pi \gets (c, s)\)
...
\href{https://eprint.iacr.org/2023/002}{BCHSV23} {[}2{]}, reduced to
EC-VRF form by setting the blinding factor \(b = 0\) and using
\(pk = sk \cdot G\) directly (see remark on page 13 of the paper).

The scheme merges the Schnorr key pair \((G, Y)\) and VRF I/O pairs into
a single DLEQ relation via delinearization, then proves it with a
...
\item
  \(Y \gets x \cdot G\)
\item
  \(\overline{io}' \gets [(G, Y)] \;\Vert\; \overline{io}\) (prepend
  Schnorr pair)
\item
  \((T, (I_m, O_m)) \gets \texttt{vrf\_transcript}(\texttt{ThinVrf}, \overline{io}', ad)\)
\item
  \(k \gets \texttt{nonce}(x, T.\texttt{fork}())\)
\item
  \(R \gets k \cdot I_m\)
...
\item
  ...
  invalid or the identity.
\item
  \(\overline{io}' \gets [(G, Y)] \;\Vert\; \overline{io}\)
\item
  \((T, (I_m, O_m)) \gets \texttt{vrf\_transcript}(\texttt{ThinVrf}, \overline{io}', ad)\)
\item
  \(c \gets \texttt{challenge}([R], T)\)
...
\item
  ...
  \item
    ...
    invalid or the identity.
  \item
    \(\overline{io}'_j \gets [(G, Y_j)] \;\Vert\; \overline{io}_j\)
  \item
    \((T_j, (I_{m,j}, O_{m,j})) \gets \texttt{vrf\_transcript}(\texttt{ThinVrf}, \overline{io}'_j, ad_j)\)
  \item
...
\texttt{hash\_to\_curve} function as described in section 1.5.2 with
...
\item
  \(T.\texttt{absorb}(seed)\)
\item
  If \(cnt > 0\): \(T.\texttt{absorb}(cnt)\) where \(cnt\) is encoded as
  a single octet
\item
  \(sk \gets \texttt{dec\_scalar\_mod}(seed)\)
\item
  \(secret \gets \texttt{nonce}(sk, T)\)
\item
  If \(secret = 0\): increment \(cnt\) and go to step 2
\item
  Return \(secret\)
\end{enumerate}

The seed is absorbed into the transcript and also passed as a scalar to
the \(\texttt{nonce}\) procedure (section 1.5.6), ensuring seed entropy
...
using the nonce function (section 1.5.6) with a distinct domain
...
from \((sk, T)\), two Pedersen VRF proofs with the same secret key, I/O
...
\item
  \(b \gets \texttt{nonce}(sk, T)\)
\end{enumerate}

\section{Appendix B. HashTranscript
Construction}\label{appendix-b.-hashtranscript-construction}

Concrete instantiation of the transcript interface (section 1.5.1) using
SHA-512.
...
1.5.5) sets the merged pair to the identity:
