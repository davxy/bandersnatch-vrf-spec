...
\emph{Clone}: duplicates the full internal state (including any partial
...
\item
  ...
  \item
    If \(n \geq 2\): \(T' \gets T.\texttt{clone}()\),
    \(T'.\texttt{absorb}(0\text{x04} \;\Vert\; n \text{ as u32 LE})\),
    squeeze \(n\) scalars
    \(z_i \gets \texttt{from\_le\_bytes\_mod\_order}(T'.\texttt{squeeze}(\text{CHALLENGE\_LEN}))\),
...
\item
  \(T'.\texttt{absorb}(0\text{x05} \;\Vert\; \texttt{serialize}(sk))\)
\item
  \(h \gets T'.\texttt{squeeze}(64)\)
\item
  \(T.\texttt{absorb}(0\text{x06} \;\Vert\; h[32..64])\)
\item
  \(k \gets \texttt{from\_le\_bytes\_mod\_order}(T.\texttt{squeeze}(\text{expanded\_scalar\_len}))\)
\end{enumerate}

Note: \(T\) is consumed (mutated then squeezed). Callers must pass
clones where the transcript is needed afterwards.
...
\tightlist
\item
  \(T.\texttt{absorb}(0\text{x02})\)
\item
  For each \(P_i\) in \(\bar{P}\):
  \(T.\texttt{absorb}(\texttt{serialize}(P_i))\)
\item
  \(c \gets \texttt{from\_le\_bytes\_mod\_order}(T.\texttt{squeeze}(\text{CHALLENGE\_LEN}))\)
\end{enumerate}
...
\item
  \(k \gets \texttt{nonce}(x, T.\texttt{clone}())\)
\item
...
\item
  \((T, (I_m, O_m)) \gets \texttt{vrf\_transcript}(\overline{io}, ad)\)
\item
  \(b \gets \texttt{blinding}(x, T.\texttt{clone}())\) (see Appendix
  A.2)
\item
  \(\bar{Y} \gets x \cdot G + b \cdot B\)
\item
  \(T_k \gets T.\texttt{clone}()\),
  \(\quad T_k.\texttt{absorb}(\texttt{serialize}(b))\),
  \(\quad k \gets \texttt{nonce}(x, T_k)\)
\item
  \(T_{kb} \gets T.\texttt{clone}()\),
