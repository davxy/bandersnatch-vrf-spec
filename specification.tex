...
\item
  \(\texttt{serialize}(s)\): Little-endian scalar encoding (section
  2.1).
\item
...
\(\texttt{hash\_to\_curve}\) method defined in section 3 of
\href{https://datatracker.ietf.org/doc/rfc9380}{RFC-9380} {[}5{]},
instantiated with the \emph{Elligator 2} map to curve (section 6.8.2)
and \(\texttt{expand\_message\_xmd}\) with SHA-512 (section 5.3.1).
...
\subsection{1.9. Transcript}\label{transcript}
...
\subsection{1.10. VRF Transcript}\label{vrf-transcript}
...
\subsection{1.11. Nonce Procedure}\label{nonce-procedure}
...
\subsection{1.12. Challenge Procedure}\label{challenge-procedure}
...
Configuration follows the \emph{``cipher suite''} guidelines defined in
section 5.5 of
...
\item
  ...
  \end{itemize}
\item
  \texttt{challenge\_len} = 16. This value provides 128 bits of
  security, matching the effective security level of the Bandersnatch
  elliptic curve, and it ensures that the statistical bias during the
  modular reduction of the challenge is negligible.
\item
  \texttt{expanded\_scalar\_len} = 48. Computed as
  \(\lceil(\lceil\log_2(r)\rceil + 128) / 8\rceil\), ensuring negligible
  bias during modular reduction for nonce scalars.
\item
...
\item
  The transcript is
  \texttt{HashTranscript\textless{}SHA-512\textgreater{}} as defined in
  section 1.9.
\item
  Nonce generation uses the transcript-based procedure in section 1.11.
\item
  Challenge generation uses the transcript-based procedure in section
  1.12.
\item
  The \texttt{int\_to\_string} (equivalently \(\texttt{serialize}\) for
  scalars) function encodes into the 32 octets little endian
  representation.
\item
  The \texttt{string\_to\_int} function decodes an octet-string as a
  little-endian integer eventually reducing modulo the prime field
  order.
\item
  The \texttt{point\_to\_string} (equivalently \(\texttt{serialize}\)
  for points) function converts a point in \(\bold{G}\) to an
  octet-string using compressed form. The \(y\) coordinate is encoded
  using \texttt{int\_to\_string} function and the most significant bit
  of the last octet is used to keep track of \(x\) sign. This implies
  that \texttt{ptLen\ =\ flen\ =\ 32}.
\item
  ...
  procedure. This function MUST output ``INVALID'' if the octet-string
...
\item
  ...
  with \texttt{hLen} = 64.
\item
  The \texttt{ECVRF\_encode\_to\_curve} function uses the
  \emph{Elligator 2} hash-to-curve construction described in section
  1.4, following section 5.4.1.2 of
  \href{https://datatracker.ietf.org/doc/rfc9381}{RFC-9381}.
\end{itemize}
...
Refer to section 2.1 for shared parameters, serialization, and
hash-to-curve configuration.

\subsection{3.2. Prove}\label{prove-1}
...
\subsection{5.1. Configuration}\label{configuration-3}
...
\subsection{5.2. Prove}\label{prove-3}
...
\subsection{5.3. Verify}\label{verify-3}
...
The \(\texttt{nonce}\) procedure (section 1.11) is called with a zero
...
the nonce function (section 1.11) with a distinct domain separator.
...
after the nonce procedure change (section 1.9) is applied to the
