...
\item
  ...
  significant bit of the last octet encodes the sign of \(x\). This
  gives \texttt{ptLen} = \texttt{fLen} = \(32\).
\item
...
output point using a transcript-based point-to-hash procedure.
...
\item
  \(T.\texttt{absorb}(\texttt{Nonce} \;\Vert\; h[32..64])\)
\item
...
\texttt{ECVRF\_encode\_to\_curve} function as described in section 1.6
with input the string: \texttt{"pedersen-blinding"}.
...
\item
  ...
  A point with unknown discrete logarithm derived using the
  \texttt{ECVRF\_encode\_to\_curve} function as described in section 1.6
  with input the string: \texttt{"ring-accumulator"}.
\item
  ...
  A point with unknown discrete logarithm derived using the
  \texttt{ECVRF\_encode\_to\_curve} function as described in section 1.6
  with input the string: \texttt{"ring-padding"}.
\item
