...
\subsection{1.3. Encoding}\label{encoding}

\begin{itemize}
\tightlist
\item
  \(\texttt{enc\_point}(P)\): Encodes a point in compressed form. The
  \(y\) coordinate is serialized in little-endian and the most
  significant bit of the last octet encodes the sign of the
  \(x\)-coordinate. This gives \texttt{ptLen} = \texttt{fLen} = \(32\).
\item
  \(\texttt{enc\_scalar}(s)\): Encodes a scalar into 32 octets in
  little-endian representation.
\item
  \(\texttt{enc\_32}(n)\): Encode integer \(n\) as a 4-byte
  little-endian octet string.
\item
  \(\texttt{dec\_scalar}(buf)\): Interpret octet string \(buf\) as a
  little-endian integer and reduce modulo the prime field order \(r\).
\end{itemize}

Point deserialization MUST output ``INVALID'' if the octet-string does
not decode to a point on the prime subgroup \(\bold{G}\). All point
arithmetic MUST be performed in \(\bold{G}\). Points not in \(\bold{G}\)
MUST be rejected at all entry points; accepting a point on the full
curve but outside the prime-order subgroup enables small-subgroup
attacks that break the VRF relation.
...
\subsection{1.5. Secret Key Generation}\label{secret-key-generation}

Implementations should provide a method for deterministic secret
generation from seeds. One RECOMMENDED method is described in Appendix
A. However, any secure method that outputs uniformly random scalars in
\(\bold{F}\)\hspace{0pt} is acceptable.

The secret key scalar MUST NOT be zero. A zero secret key maps every VRF
input to the identity point, making all outputs trivially predictable
and proofs forgeable.

\subsection{1.6. VRF Input}\label{vrf-input}
...
\subsection{1.7. VRF Output}\label{vrf-output}
...
\subsection{1.8. Additional Data}\label{additional-data}
...
produced VRF output. The length of \(ad\) MUST NOT exceed \(2^{32} - 1\)
bytes, as the length is encoded via \(\texttt{enc\_32}\) in the
transcript (section 1.11).

\subsection{1.9. VRF-AD}\label{vrf-ad}
...
Where \(\overline{io}\) is a sequence of \((I_i, O_i)\) input/output
pairs.

\subsection{1.10. Transcript}\label{transcript}
...
\tightlist
\item
  \(\texttt{new\_transcript}()\): Create a fresh transcript instance and
  absorb \(\texttt{suite\_id}\).
\item
  \(\texttt{absorb}(data)\): Feed bytes into the hash state. MUST NOT be
  called after squeeze.
\item
  \(\texttt{squeeze}(n) \to \bold{\Sigma}^n\): Produce \(n\) output
  bytes.
\item
...
\subsection{1.11. VRF Transcript}\label{vrf-transcript}
...
\item
  ...
  \item
    If \(n \geq 2\): \(T' \gets T.\texttt{fork}()\),
    \(T'.\texttt{absorb}(\texttt{Delinearize})\), squeeze \(n\) scalars
    \(z_i \gets \texttt{dec\_scalar}(T'.\texttt{squeeze}(\texttt{challenge\_len}))\),
...
\subsection{1.12. Nonce Procedure}\label{nonce-procedure}
...
\item
  \(k \gets \texttt{dec\_scalar}(T.\texttt{squeeze}(\text{expanded\_scalar\_len}))\)
\item
...
\subsection{1.13. Challenge Procedure}\label{challenge-procedure}
...
\item
  \(c \gets \texttt{dec\_scalar}(T.\texttt{squeeze}(\texttt{challenge\_len}))\)
\end{enumerate}
...
The challenge scalar \(c\) is serialized as the first
\texttt{challenge\_len} bytes of its little-endian representation,
giving a total proof size of \texttt{challenge\_len} + \texttt{fLen} =
48 bytes. On deserialization, \(c\) is interpreted as a little-endian
integer with no modular reduction (the value is always less than \(r\)).
...
The \emph{blinding base} \(B \in \bold{G}\) is defined as follows:
...
\texttt{hash\_to\_curve} function as described in section 1.6 with input
the string: \texttt{"pedersen-blinding"}.
...
\item
  ...
  A point with unknown discrete logarithm derived using the
  \texttt{hash\_to\_curve} function as described in section 1.6 with
...
\item
  ...
  a single octet
\item
  \(sk \gets \texttt{dec\_scalar}(seed)\)
\item
...
the \(\texttt{nonce}\) procedure (section 1.12), ensuring seed entropy
...
using the nonce function (section 1.12) with a distinct domain
...
1.11) sets the merged pair to the identity:
...
\textbf{TODO}: The test vectors below are stale and must be regenerated
after the nonce procedure change (section 1.10) is applied to the
