...
(\texttt{HashTranscript\textless{}SHA-512\textgreater{}}):

\emph{Initialization}: \(\texttt{new\_transcript}()\) creates a fresh
SHA-512 state and feeds \(\texttt{suite\_id}\) into it.

\emph{Absorb}: feeds raw bytes directly into the SHA-512 state.
...
\emph{Squeeze} (counter-mode XOF): on the first squeeze call, finalize
...
\emph{Fork}: duplicates the full internal state (including any partial
